...
#cv2へのCSIカメラからのキャプチャ指示のGStreamerコマンド文字列
...
#キャプチャーのフレームレート
FPS = 30
#キャプチャー映像を出力するウィンドウの名前
WINDOW_NAME = 'Human Sensor'
#YOLO v2のNNへの入力画像のサイズ
YOLO2_NN_INPUT_SIZE = (416, 416)
#ONNXモデルファイルのパス
ONNX_FILE_PATH = './tyolov2NN_trained/tyolov2NN_trained.onnx'
#TensorRTランタイムファイルのパス
TRT_ENGINE_FILE_PATH = './tyolov2NN_trained/tyolov2NN_trained.trt'
#定義済みクラスラベルファイルのパス
DEF_CLASSES_FILE_PATH = './tyolov2NN_trained/voc.names'
#LEDのアノード（+）に抵抗を経由してつながるピン番号（Rasbperry Pi GPIO番号であってボード上の番号ではない）
PIN_NUM_LED_PLUS = 18
#YOLO v2のNNの本来の出力shape（ただしTRTの出力はこの通りではないことに注意）
YOLO2_NN_OUTPUT_SHAPE = (1, 125, 13, 13)
#訓練済みTiny YOLO v2のNNの訓練時に決定されたアンカー
TYOLO2_ANCHORS = [(1.08, 1.19), (3.42, 4.41), (6.63, 11.38), (9.42, 5.11), (16.62, 10.52)]
...
def draw_bboxes_on_cap_img(cap_img_cv2, bboxes, bboxes_score, bboxes_class_index, defined_classes):
    ...
    for bbox, bbox_score, bbox_class_index in zip(bboxes, bboxes_score, bboxes_class_index):
        ...
        left_bbox = max(0, round(bx + 0.5))
        right_bbox = min(cap_img_cv2_width, round(bx + bw + 0.5))
        top_bbox = max(0, round(by + 0.5))        
        bottom_bbox = min(cap_img_cv2_height, round(by + bh + 0.5))
        
        #実際にbboxを描画
        cv2.rectangle(cap_img_cv2, (left_bbox, top_bbox), (right_bbox, bottom_bbox), (0, 0, 255), 3)
        ...
        indicate_message_on_cap_img(cap_img_cv2, class_label_score_str, right_bbox, top_bbox)
        #端末にも表示
...
def main():
    ...
    parser.add_argument('--flip_method', \
        ...
        help='Capture image rotation')
    parser.add_argument('--objth', \
        type=float, default=0.6, metavar='OBJ_THRESH', \
        help='Threshold of bbox score to filter')
    parser.add_argument('--nmsth', \
        type=float, default=0.3, metavar='NMS_THRESH', \
        help='Threshold of bboxes IOU to apply NMS')
    parser.add_argument('--class_led', \
        type=int, default=14, metavar='CLASS_LED', \
        help='Class index to turn on LED')
    ...
    obj_threshold = args.objth #規定量のbboxesをスコアで足切りする時のスコアの閾値
    nms_threshold = args.nmsth #検出されたbboxesに非最大値抑制を適用する時のIOUの閾値
    class_index_to_notify = args.class_led #LEDを点灯させたいクラスのインデックス
    ...
    print("args.class_led:", class_index_to_notify)
    ...
    try:
        ...
        try:
            #LEDを点灯させて通知したいクラスのラベルを取得
            class_label_to_notify = defined_classes[class_index_to_notify]
        except Exception as e:
            class_label_to_notify = "不明"
        ...
        with get_engine(ONNX_FILE_PATH, TRT_ENGINE_FILE_PATH) as engine, engine.create_execution_context() as context:
            ...
            while True:
                ...
                #YOLO v2の本来の出力形状にする。
                output_NN =output_from_trt[0].reshape(YOLO2_NN_OUTPUT_SHAPE) #(1, 125, 13, 13)
